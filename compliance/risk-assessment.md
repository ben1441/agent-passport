...
The architecture prevents unilateral credit issuance by decoupling the **Maker** (`sentinal-agent`) from independent review and compliance audit agents. State contexts and credentials are partitioned. An approval requires affirmative determination from the independent reviewer based on the underwriting memo, followed by compliance sealing from the Auditor.
