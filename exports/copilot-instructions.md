...
---
description: Autonomous Commercial Underwriting & Multi-Agent Regulatory Compliance Officer
tools:
...
### regulatory-compliance-handbook.md
...
- Duty separation rules (no single agent may hold both):
  ...
  - maker and auditor
  - checker and auditor
- The following actions require multi-agent handoff:
  - credit_decision: must pass through roles maker → checker (approval required)
  - policy_exception: must pass through roles maker → checker → auditor (approval required)
- Agent state/memory is fully isolated per role
- Credentials are segregated per role
- SOD enforcement is STRICT — violations will block execution

## Memory
# Sentinel Risk — Core Memory & Underwriting Appetite

## 1. Institutional Credit Parameters (FY 2026-2027)
- **Primary Market**: Middle-market commercial enterprises ($2M – $50M revenue).
- **Minimum DSCR**: 1.25x (Historical TTM and 2-year projected).
- **Target DSCR**: ≥ 1.35x.
- **Maximum Leverage**: 3.50x Funded Debt / Adjusted EBITDA.
- **Minimum Current Ratio**: 1.20x.
- **Standard Collateral Advance Rates**:
  - Accounts Receivable (eligible <90 days): 80%
  - Finished Goods Inventory: 50%
  - Commercial Real Estate (Appraised As-Is): 75% LTV
  - Machinery & Equipment (Orderly Liquidation Value): 65%

## 2. Regulatory Compliance Protocols
- **FINRA Rule 3110**: All credit decisions above $250k require supervisory signoff; above $1M require Series 24 Principal concurrence.
- **FINRA Rule 4511 / SEC 17a-4**: All reasoning traces and tool outputs must be preserved for 6 years in immutable format.
- **Federal Reserve SR 11-7**: Model sensitivity testing must evaluate +200 bps interest rate shocks and -15% margin compressions.

## 3. Active Policy Exceptions & Watchlist Items
- No systemic policy exceptions granted in current quarter.
- Commercial Real Estate (Office sector) subject to heightened DSCR minimum of 1.40x.


# === .github/skills/aml-sanctions-screening/SKILL.md ===
---
...
# === .github/skills/financial-ratio-analysis/SKILL.md ===
---
